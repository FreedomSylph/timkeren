...
public class GenieMadness : MonoBehaviour {
	...
	void Start()
	{
		...
		countEnemiesHit = 0;
		markedTargetName = "GenieMadnessVictim";
	}
	...
	void setFirstEnemyOnTap()
	{
		...
		//GetComponent<Animator>().Play("Jin_Active");
		groundClicked = true;
		...
		//play dukun tapping 
		heroSkillTrigger.getAnimator ().Play ("Dukun_Tapping");
		//place the instantiate of RedJinProjectileHolder
		this.transform.position = new Vector3(ground.x,-1.5f,10);
